...
import { getActorProfile, assertSectionInScope, assertStudentInScope, assertCampusInScope } from "../../lib/scope.js";
...
export async function enterResultItemHandler(req: Request, res: Response): Promise<void> {
  ...
  const profile = await getActorProfile(req.user!.id);
  await assertSectionInScope(profile, await service.getResultSectionId(req.params.resultId!));
...
export async function requestResultCorrectionHandler(req: Request, res: Response): Promise<void> {
  ...
  const profile = await getActorProfile(req.user!.id);
  await assertSectionInScope(profile, await service.getResultItemSectionId(req.params.itemId!));
