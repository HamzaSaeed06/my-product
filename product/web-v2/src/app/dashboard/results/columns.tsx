...
// read-only; opening a result (click the student's name) is where marks
// get entered/edited and the status gets advanced, exactly like
// Assessments' list+detail split.
export const resultColumns: ColumnDef<ResultTableRow>[] = [
  {
    ...
    meta: { label: "Student" },
    cell: ({ row }) => (
      <Link href={`/dashboard/results/${row.original.id}`} className="font-medium text-foreground hover:underline">
